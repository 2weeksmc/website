...
import SliderComponent from '@/components/pricing/SliderComponent.vue'
  ...
  <main class="w-full min-h-[calc(100svh-21rem)] text-white">
    ...
    <div class="w-full flex flex-col items-center">
      <div class="my-16 pr-8 w-3/4 min-h-100 flex flex-col gap-8 bg-[#1E1E1E] rounded-2xl">
        ...
        <p class="w-full text-center text-2xl font-semibold">Configurator</p>
        <div class="mt-2 mb-4 px-8 w-full">
          <SliderComponent label="RAM" :min="2" :max="16" :step="2" v-model="ram" />
          <SliderComponent label="NVMe Storage" :min="16" :max="128" :step="16" v-model="storage" />
          <SliderComponent label="Plugins" :min="0" :max="50" :step="5" v-model="plugins" />
          <SliderComponent label="Mods" :min="0" :max="100" :step="5" v-model="mods" />
        </div>
      </div>
    </div>
  </main>
